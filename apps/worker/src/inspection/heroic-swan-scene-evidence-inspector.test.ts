        ...
        : options.runJsxResult === "hostLayerMatches"
          ...
          : options.runJsxResult === "compositionSummary"
            ...
            : options.runJsxResult === "layerTransforms"
              ...
              : options.runJsxResult === "layerAtTime"
                ? `return { content: [{ type: "text", text: JSON.stringify({ result: JSON.stringify({
                    ok: true,
...
describe("HeroicSwanSceneEvidenceInspector - real spawned MCP server, not mocked", () => {
  ...
  it("real 2026-09-17: describeLayerAtTime sends the read-only describe-layer-at-time script for exactly that layer and time, and parses its facts into the response", async () => {
    const capturePath = join(dir, "received-jsx-code-layer-at-time.txt");
    await writeFakeServer(dir, { runJsxResult: "layerAtTime", captureReceivedJsxCodeToFile: capturePath });
    const inspector = new HeroicSwanSceneEvidenceInspector({ aeMcpPath: dir });
    const result = (await inspector.inspect(baseRequest({ describeLayerAtTime: { layerIndex: 2, timeSeconds: 4.96 } }))) as SceneEvidenceSuccess;

    const receivedCode = await readFile(capturePath, "utf8");
    expect(receivedCode).toMatch(/DYO DESCRIBE_LAYER_AT_TIME/);
    ...
    expect(result.kind).toBe("evidence");
    expect(result.response.layerAtTimeFactsFailureReason).toBeNull();
    expect(result.response.layerAtTimeFacts).toMatchObject({ timeSeconds: 4.96, layer: { layerIndex: 2, layerName: "Text 1" }, text: { justification: "RIGHT_JUSTIFY" } });
  });

  it("real 2026-09-17: a request without describeLayerAtTime has neither new response key - its shape is exactly as before", async () => {
    await writeFakeServer(dir, { runJsxResult: "success" });
    const inspector = new HeroicSwanSceneEvidenceInspector({ aeMcpPath: dir });
    const result = (await inspector.inspect(baseRequest())) as SceneEvidenceSuccess;

    expect(result.kind).toBe("evidence");
    expect(result.response).not.toHaveProperty("layerAtTimeFacts");
    expect(result.response).not.toHaveProperty("layerAtTimeFactsFailureReason");
  });

  it("real 2026-09-17: reports layerAtTimeFactsFailureReason (never fabricated facts) when the describe-layer-at-time script call fails", async () => {
    await writeFakeServer(dir, { runJsxResult: "error" });
    const inspector = new HeroicSwanSceneEvidenceInspector({ aeMcpPath: dir });
    const result = (await inspector.inspect(baseRequest({ describeLayerAtTime: { layerIndex: 2, timeSeconds: 4.96 } }))) as SceneEvidenceSuccess;
