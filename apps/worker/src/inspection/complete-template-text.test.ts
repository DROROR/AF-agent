...
import { MAX_ASSEMBLED_TEXT_CODE_UNITS, readCompleteTextVerification, type RunSliceScript } from "./complete-template-text.js";
...
describe("readCompleteTextVerification", () => {
  it("reassembles a long text from slices and digests the COMPLETE string", async () => {
  ...
  it("reassembles an astral character split across a slice boundary, byte for byte", async () => {
  ...
  it("refuses a pathologically long text rather than assembling it", async () => {
    const server = serving("short", { totalOverride: () => MAX_ASSEMBLED_TEXT_CODE_UNITS + 1 });

    const result = await readCompleteTextVerification(server.run, { ...TARGET, expectedCodeUnitLength: MAX_ASSEMBLED_TEXT_CODE_UNITS + 1 });

    expect(result.ok).toBe(false);
    expect(result.ok === false && result.reason).toMatch(/beyond the .* this worker will assemble/);
  });
