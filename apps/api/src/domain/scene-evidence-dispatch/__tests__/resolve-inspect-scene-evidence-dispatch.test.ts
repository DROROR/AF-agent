...
describe("resolveInspectSceneEvidenceDispatch - previewTimingDiscoverCompositionId + previewTimingDiscoverLayerDetails (real 2026-09-11 nested-content audit, session a7fee3d9)", () => {
  ...
  it("without any sub-mode flag, still requires previewTimingFindHostLayersChildCompositionId - the updated four-way error message names all four sub-modes", () => {
    const result = resolveInspectSceneEvidenceDispatch({
      scenePlanId: "scene-1",
      currentPlan: validPlan(),
      currentProjectManifest: outerManifest,
    ...
    expect(result.reason).toContain("previewTimingDescribeLayerTransforms");
  });
});

describe("resolveInspectSceneEvidenceDispatch - previewTimingDiscoverCompositionId + previewTimingDescribeLayerTransforms (real 2026-09-11 nested-content audit, session a7fee3d9)", () => {
  const outerManifest = validManifest({
    compositions: [
      { compositionId: "comp-1", aeProjectItemIndex: 14, name: "Scene 1", widthPx: 1920, heightPx: 1080, durationSeconds: 7.007, frameRate: 29.97, isNestedOnlyReferenced: false, parentCompositionIds: [] },
      { compositionId: "comp-1600", aeProjectItemIndex: 30, name: "Pre-comp 2", widthPx: 1920, heightPx: 1080, durationSeconds: 7.007, frameRate: 29.97, isNestedOnlyReferenced: true, parentCompositionIds: ["comp-1"] }
  ...
  it("resolves the requested composition identity, with zero server-supplied layerIndices (self-sufficient) and describeLayerTransforms: true - never the other three sub-modes", () => {
    const result = resolveInspectSceneEvidenceDispatch({
      scenePlanId: "scene-1",
      currentPlan: validPlan(),
      currentProjectManifest: outerManifest,
    ...
    expect(result.payload.compositionName).toBe("Pre-comp 2");
    expect(result.payload.layerIndices).toEqual([]);
    expect(result.payload.describeLayerTransforms).toBe(true);
    expect(result.payload.describeCompositionSummary).toBeUndefined();
    expect(result.payload.discoverLayerDetails).toBeUndefined();
    expect(result.payload.findHostLayersForChildCompositionId).toBeUndefined();
  });

  it("takes priority over previewTimingFindHostLayersChildCompositionId when both are somehow present (and no earlier sub-mode is present) - checked before the host-layer-lookup branch", () => {
    const result = resolveInspectSceneEvidenceDispatch({
      scenePlanId: "scene-1",
      currentPlan: validPlan(),
      currentProjectManifest: outerManifest,
  ...
  it("refuses an unknown previewTimingDiscoverCompositionId - the same manifest-validated addressing safety net as the other three sub-modes", () => {
    const result = resolveInspectSceneEvidenceDispatch({
      scenePlanId: "scene-1",
      currentPlan: validPlan(),
      currentProjectManifest: outerManifest,
