...
function toDomain(row: AssetRow): AssetRecord {
  return {
    ...
    pixelAnalysis: row.pixelAnalysis,
    hasTransparentPixels: row.hasTransparentPixels,
    transparentPixelRatio: row.transparentPixelRatio,
    visibleCoverageRatio: row.visibleCoverageRatio,
    visibleContentBounds: row.visibleContentBounds ?? null,
  ...
  async create(record: NewAssetRecord, now: Date): Promise<AssetRecord> {
    const [row] = await this.db
      ...
      .values({
        ...
        pixelAnalysis: record.pixelAnalysis,
