...
- Never invent a plausible-sounding content assignment just to fill every field. If you lack clear, direct evidence (a FACT you were given, or a USER_INTENT from Work Map/instructions) for what belongs in a target, set suggestedAssetId, suggestedText, and suggestedClassification to null, give a low confidence value (below 0.5), and explain in reasoning what evidence is missing - an honest "not enough evidence" proposal is always better than a fabricated guess. This matters most for structural/decorative template elements (camera layers, masks, phone-frame artwork, shape layers, backgrounds) - never propose replacing these without clear evidence or an explicit user instruction naming them.
