...
const EXISTING_LANDSCAPE_RENDER_ID = "84f630a8-c35c-45bd-ae41-e180e91ed4d3";
...
async function main(): Promise<void> {
  ...
  if (config.env !== "sandbox") {
    throw new Error(`Refusing to run: SHOTSTACK_ENV="${config.env}"`);
  }
  const client = new ShotstackClient(config);

  async function pollExisting(label: string, id: string) {
    console.log(`Polling existing ${label} render id=${id}...`);
    ...
    console.log(`${label} RESULT: TIMED OUT.`);
    return { id, status: "timeout", url: null };
  }

  async function submitAndPoll(label: string, outputKind: "LANDSCAPE" | "REELS", sceneMap: SceneMap) {
    ...
    return pollExisting(label, id);
  }

  interface UploadedUrls {
    logoCognetica: string;
    appStoreBadges: string;
    openingVideo: string;
    frame1Photo: string;
    sharedRecording: string;
    frame4Video: string;
    frame5Photo: string;
    frame6RightVideo: string;
  }
  const urls = JSON.parse(
    readFileSync(path.join(ASSET_DIR, "..", "uploaded-urls-cache.json"), "utf8")
  ) as UploadedUrls;
    ...
    scenes: [
      {
        ...
        startMs: 0,
        durationMs: 4500,
        assets: [{ placeholderId: "opening-video", assetType: "VIDEO", sourceUrl: urls.openingVideo, trimSeconds: 2.5 }],
        texts: [],
      ...
      },
      {
        ...
        texts: [
          ...
          { placeholderId: "branding", content: "עכשיו באפליקציה מבית DYO App", fontFamily: "Heebo", fontWeight: 700 }
        ],
        phonePosition: "LEFT",
      ...
      },
      {
        ...
        startMs: 8000,
        durationMs: 4900,
        assets: [{ placeholderId: "frame2-video", assetType: "VIDEO", sourceUrl: urls.sharedRecording, trimSeconds: 36 }],
        texts: [
      ...
      },
      {
        ...
        startMs: 12900,
        durationMs: 3100,
        assets: [{ placeholderId: "frame3-video", assetType: "VIDEO", sourceUrl: urls.sharedRecording, trimSeconds: 22 }],
      ...
      },
      {
        ...
        startMs: 16000,
        durationMs: 4650,
        assets: [{ placeholderId: "frame4-video", assetType: "VIDEO", sourceUrl: urls.frame4Video, trimSeconds: 2 }],
        texts: [
          { placeholderId: "headline", content: "חוויית למידה שמלווה אותך בכל יום", fontFamily: "Heebo", fontWeight: 700 },
      ...
      },
      {
        ...
        durationMs: 6980,
        assets: [
          { placeholderId: "frame6-left", assetType: "VIDEO", sourceUrl: urls.sharedRecording, trimSeconds: 60, offsetX: -0.28 },
          { placeholderId: "frame6-right", assetType: "VIDEO", sourceUrl: urls.frame6RightVideo, trimSeconds: 0, offsetX: 0.28 },
          { placeholderId: "logo", assetType: "LOGO", sourceUrl: urls.logoCognetica }
        ],
        texts: [
          { placeholderId: "headline", content: "קוגנטיקה - מצוינות קלינית מתחילה כאן", fontFamily: "Heebo", fontWeight: 700 },
  ...
  const landscapeResult = await pollExisting("Landscape", EXISTING_LANDSCAPE_RENDER_ID);
  const verticalResult = await submitAndPoll("Vertical/Reels", "REELS", sceneMap);
...
main().catch((error: unknown) => {
  console.error("Cognetica reference POC (poll+vertical) failed:", error instanceof Error ? error.message : error);
  process.exitCode = 1;
});
