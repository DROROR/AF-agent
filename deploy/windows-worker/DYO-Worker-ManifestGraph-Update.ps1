<#
.SYNOPSIS
  DYO Windows Worker - Preview Timing manifest-graph + early-exit
  discovery fix, for an ALREADY-REGISTERED install. Double-click
  DYO-Worker-ManifestGraph-Update.bat instead of running this file
  directly.

.DESCRIPTION
  Fixes a real 2026-09-10 incident (session a7fee3d9): "Analyze Preview
  Timing" reported "comp-1 is not reachable from comp-210 (!Render)" - a
  false negative. Root cause was two compounding bugs: the Worker's own
  layer-classification scan of the large master render composition still
  timed out even in the prior "discovery" mode update, and the dashboard's
  own graph-search code mistook that timeout for "this composition
  genuinely has no nested children" instead of "the scan failed".

  This update ships the Worker-side half of the real fix: the existing
  discoverLayerDetails read-only inspection script now accepts an
  optional "I already know which nested composition I'm looking for"
  hint. When the dashboard already knows (from the project's own already-
  confirmed manifest data) exactly which composition it expects to find,
  the Worker's own scan stops the INSTANT it finds that one composition,
  instead of exhaustively classifying every remaining layer of a
  potentially large composition - directly avoiding the timeout that
  caused the original false-negative result.
...
Write-Host "================================================"
Write-Host "  DYO Windows Worker - Preview Timing Manifest Graph Update"
Write-Host "================================================"
...
Write-Host "composition-graph search. It does not ask for a registration code,"
...
if (-not (Test-Path (Join-Path $sourceApp "dist\index.js"))) {
  ...
  Write-Host "Re-download the full DYO Worker Manifest Graph update package and try again."
  exit 1
}
...
if ($npmExitCode -ne 0) {
  ...
  Write-Host "Check your internet connection and re-run DYO-Worker-ManifestGraph-Update.bat."
  exit 1
}
...
if (Test-Path $sdkCheckPath) {
  ...
  Write-Host "Please run DYO-Worker-ManifestGraph-Update.bat again. If this keeps happening, contact DYO."
  exit 1
}
...
Write-Host "================================================"
Write-Host "DYO Worker is running with the Preview Timing manifest-graph fix, using"
