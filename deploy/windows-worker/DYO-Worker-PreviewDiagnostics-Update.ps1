<#
.SYNOPSIS
  DYO Windows Worker - Complete Preview diagnostics + output-path safety
  update, for an ALREADY-REGISTERED install. Double-click
  DYO-Worker-PreviewDiagnostics-Update.bat instead of running this file
  directly.

.DESCRIPTION
  Fixes a real 2026-09-10 incident (job dac9fc90-ed8b-4589-9fe5-d34d4f95aa95):
  a "Create Complete Preview" job failed with "expected output file does
  not exist" after aerender itself reported success (exit code 0). The
  real cause was AE's own render-settings-template name not matching a
  real Render Queue template on this machine - but AE's own diagnostic
  about that lands on stdout/stderr, not the exit code, and this Worker
  was silently discarding that output on exactly this failure path.

  This update makes two changes:
    - When a Complete Preview's expected output file genuinely does not
      exist after aerender exits 0, the real aerender log (last 2000
      characters of its own stdout/stderr) is now included in the
      reported failure - the same way a nonzero exit code already
      included a log excerpt. This closes a real diagnostic gap: a
      genuinely failed render used to report only "file does not exist",
      with no way to see AE's own explanation for why.
    - Both the Complete Preview and the Landscape/Reels render output
      filenames are now explicitly validated (non-empty, no path
      separators, not a bare extension, extension present exactly once)
      immediately before every render - defense-in-depth so a future
      change can never silently reintroduce an invalid output filename.
  ...
  This is a strictly ADDITIVE change:
    ...
    - Never changes what file aerender is told to write to, or what file
      is checked for - the real, already-correct output path contract is
      unchanged; this only makes a genuine failure more diagnosable and
      adds a safety check that never affects a normal, successful run.
    - Never touches any scene-editing or asset-mapping operation - this
      update has nothing to do with any of them.

  This script itself never runs CREATE_PREVIEW/RENDER, never connects to
...
Write-Host "================================================"
Write-Host "  DYO Windows Worker - Complete Preview Diagnostics Update"
Write-Host "================================================"
Write-Host "This updates the DYO Worker program files on this ALREADY-REGISTERED"
Write-Host "computer with a fix that surfaces the real aerender log when a Complete"
Write-Host "Preview or render fails validation. It does not ask for a registration"
Write-Host "code, does not change which DYO Worker this computer is, and does not"
Write-Host "open, modify, or run anything against any After Effects project."
Write-Host ""
...
if (-not (Test-Path (Join-Path $sourceApp "dist\index.js"))) {
  ...
  Write-Host "Re-download the full DYO Worker Complete Preview Diagnostics update package and try again."
  exit 1
}
...
if ($npmExitCode -ne 0) {
  ...
  Write-Host "Check your internet connection and re-run DYO-Worker-PreviewDiagnostics-Update.bat."
  exit 1
}
...
if (Test-Path $sdkCheckPath) {
  ...
  Write-Host "Please run DYO-Worker-PreviewDiagnostics-Update.bat again. If this keeps happening, contact DYO."
  exit 1
}
...
# run CREATE_PREVIEW/RENDER, ae-mcp health, or anything else against
...
Write-Host "================================================"
Write-Host "DYO Worker is running with the Complete Preview diagnostics fix, using"
