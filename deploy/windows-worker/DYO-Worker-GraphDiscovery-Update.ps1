<#
.SYNOPSIS
  DYO Windows Worker - Preview Timing composition-graph discovery
  performance fix, for an ALREADY-REGISTERED install. Double-click
  DYO-Worker-GraphDiscovery-Update.bat instead of running this file
  directly.

.DESCRIPTION
  Fixes a real 2026-09-10 incident (session a7fee3d9): "Analyze Preview
  Timing" failed with "Could not discover which layer in comp-210 hosts
  comp-1". The real cause was NOT a missing composition-graph edge - it
  was this Worker's own layer-inspection script timing out over MCP while
  scanning the master !Render composition's own ~40+ layers, each with
  four expensive AE engine property reads (stretch/timeRemapEnabled/
  opacity/sourceText) that graph DISCOVERY never actually needed.

  This update adds a lightweight "discovery" mode to the EXISTING
  discoverLayerDetails read-only inspection script (already installed by
  the Preview Timing / Preview Timing Opacity updates): when the
  dashboard's own "Analyze Preview Timing" action needs to search the
  REAL composition-nesting graph (rather than measure a specific already-
  known layer's own timing), it now uses this fast mode, which skips
  those four expensive reads entirely and only classifies each layer's
  type and real nested-composition identity - safe against a composition
  of any size. A mapping's own already-known nested layers (e.g. inside a
  logo or text precomp) still get the full, detailed scan exactly as
  before.

  This is a strictly ADDITIVE change to an existing read-only script:
    - No new capability, no new allowlisted tool, no new job type.
    - Never writes, saves, or mutates anything - discoverLayerDetails was
      already read-only, and stays read-only in both modes.
...
Write-Host "================================================"
Write-Host "  DYO Windows Worker - Preview Timing Graph Discovery Update"
Write-Host "================================================"
Write-Host "This updates the DYO Worker program files on this ALREADY-REGISTERED"
Write-Host "computer with a performance fix for the Preview Timing Analysis"
Write-Host "feature's composition-graph search. It does not ask for a registration"
...
if (-not (Test-Path (Join-Path $sourceApp "dist\index.js"))) {
  ...
  Write-Host "Re-download the full DYO Worker Graph Discovery update package and try again."
  exit 1
}
...
if ($npmExitCode -ne 0) {
  ...
  Write-Host "Check your internet connection and re-run DYO-Worker-GraphDiscovery-Update.bat."
  exit 1
}
...
if (Test-Path $sdkCheckPath) {
  ...
  Write-Host "Please run DYO-Worker-GraphDiscovery-Update.bat again. If this keeps happening, contact DYO."
  exit 1
}
...
Write-Host "================================================"
Write-Host "DYO Worker is running with the Preview Timing graph-discovery performance"
Write-Host "fix, using the same DYO Worker identity this computer already had - no new"
