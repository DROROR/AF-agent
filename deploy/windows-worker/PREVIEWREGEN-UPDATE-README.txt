DYO WINDOWS WORKER - FIRST PREVIEW REGENERATION UPDATE
==============================================================
...
2. Double-click DYO-Worker-PreviewRegen-Update.bat
...
WHAT THIS UPDATES
-------------------
Fixes a real incident: the dashboard's "Regenerate First Preview" action
(recapturing a preview frame from an already-mutated, already-safety-
verified working copy, with ZERO new edits) was correctly accepted by the
server, but this computer's Worker build was older than that feature and
rejected the job outright with an INVALID_PAYLOAD error. This update adds
support for that exact job shape - it does not change how any other
EXECUTE_FRAME job (a real MAP_FOOTAGE/SET_TEXT edit) behaves at all.
