<#
.SYNOPSIS
  DYO Windows Worker - Preview Timing Analysis opacity extension update,
  for an ALREADY-REGISTERED install. Double-click
  DYO-Worker-OpacityTiming-Update.bat instead of running this file
  directly.

.DESCRIPTION
  Ships the SECOND half of the arbitrary-depth Preview Timing Analysis
  extension. Real incident (2026-09-10, session a7fee3d9, recommended
  timestamp 4.93827160493827s): a recommended First Preview timestamp
  rendered correctly but showed no logo or Hebrew branding. Two real gaps
  caused it - the first (arbitrary nested-chain depth) needed no Worker
  change at all; this update ships the second: the real Hebrew text
  layer's own opacity was confirmed 0, but opacity was only ever
  collected as evidence, never factored into the visibility calculation.

  This update makes the EXISTING discoverLayerDetails read-only inspection
  script (already shipped by DYO-Worker-PreviewTiming-Update.ps1) also
  read each layer's real `layer.opacity`: a single static value when the
  property is NOT animated (`numKeys === 0`), or the REAL, ordered
  keyframe list (every keyframe's own real time + value) when it IS
  animated - never a single sampled value standing in for a layer that
  actually fades in or out.
  ...
  This is a strictly ADDITIVE change to an existing read-only script:
    ...
    - Never writes, saves, or mutates anything - discoverLayerDetails was
      already read-only, and stays read-only. This update only READS
      layer.opacity, it never sets it.
...
Write-Host "================================================"
Write-Host "  DYO Windows Worker - Preview Timing Opacity Update"
Write-Host "================================================"
Write-Host "This updates the DYO Worker program files on this ALREADY-REGISTERED"
Write-Host "computer with the Preview Timing Analysis feature's opacity evidence"
Write-Host "extension. It does not ask for a registration code, does not change"
...
if (-not (Test-Path (Join-Path $sourceApp "dist\index.js"))) {
  ...
  Write-Host "Re-download the full DYO Worker Preview Timing Opacity update package and try again."
  exit 1
}
...
if ($npmExitCode -ne 0) {
  ...
  Write-Host "Check your internet connection and re-run DYO-Worker-OpacityTiming-Update.bat."
  exit 1
}
...
if (Test-Path $sdkCheckPath) {
  ...
  Write-Host "Please run DYO-Worker-OpacityTiming-Update.bat again. If this keeps happening, contact DYO."
  exit 1
}
...
Write-Host "================================================"
Write-Host "DYO Worker is running with the Preview Timing Analysis opacity evidence"
