<#
.SYNOPSIS
  DYO Windows Worker - Preview Timing Analysis update, for an
  ALREADY-REGISTERED install. Double-click
  DYO-Worker-PreviewTiming-Update.bat instead of running this file directly.

.DESCRIPTION
  Ships the Preview Timing Analysis feature's own worker-side read-only
  evidence extension. Real incident (2026-09-09): every existing
  scene_evidence row had zero real per-layer timing data, so First Preview
  timestamps were being chosen from average scene duration instead of real
  evidence. Investigating the fix found the existing ae_get_layer tool
  never exposes `layer.stretch`/`layer.timeRemapEnabled` at all (confirmed
  by reading the real upstream ae-mcp host script) - both are required to
  correctly map a nested composition's own local timing onto the master
  !Render timeline.

  This update makes the EXISTING discoverLayerDetails read-only inspection
  script (already shipped by DYO-Worker-TextLayerDiscovery-Update.ps1, used
  only when INSPECT_SCENE_EVIDENCE explicitly requests it) additionally
  read those two real properties per layer, wrapped in the same try/catch
  pattern the rest of that script already uses - a property that cannot be
  read (e.g. a layer type that does not support time remap) is reported as
  ...
  This is a strictly ADDITIVE change to an existing read-only script:
    ...
    - Never writes, saves, or mutates anything - discoverLayerDetails was
      already read-only, and stays read-only.
...
Write-Host "================================================"
Write-Host "  DYO Windows Worker - Preview Timing Analysis Update"
Write-Host "================================================"
Write-Host "This updates the DYO Worker program files on this ALREADY-REGISTERED"
Write-Host "computer with the Preview Timing Analysis feature's read-only evidence"
Write-Host "extension. It does not ask for a registration code, does not change which"
Write-Host "DYO Worker this computer is, and does not open, modify, or run anything"
...
if (-not (Test-Path (Join-Path $sourceApp "dist\index.js"))) {
  ...
  Write-Host "Re-download the full DYO Worker Preview Timing update package and try again."
  exit 1
}
...
if ($npmExitCode -ne 0) {
  ...
  Write-Host "Check your internet connection and re-run DYO-Worker-PreviewTiming-Update.bat."
  exit 1
}
...
if (Test-Path $sdkCheckPath) {
  ...
  Write-Host "Please run DYO-Worker-PreviewTiming-Update.bat again. If this keeps happening, contact DYO."
  exit 1
}
...
Write-Host "================================================"
Write-Host "DYO Worker is running with the Preview Timing Analysis read-only evidence"
Write-Host "extension, using the same DYO Worker identity this computer already had -"
Write-Host "no new registration was created. No After Effects project was opened,"
Write-Host "changed, or run against by this update."
Write-Host ""
