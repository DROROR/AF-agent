DYO WINDOWS WORKER - COMPLETE PREVIEW DIAGNOSTICS UPDATE
==============================================================
...
2. Double-click DYO-Worker-PreviewDiagnostics-Update.bat
...
WHAT THIS UPDATES
-------------------
When a Complete Preview or render fails because the expected output video
file was never created, the failure message now includes AE's own real
log output explaining why - previously that information was captured
internally but never actually reported, making a genuine failure much
harder to diagnose. This update also adds an extra safety check on the
output filename before every render, so a future update could never
accidentally send aerender an invalid destination.
