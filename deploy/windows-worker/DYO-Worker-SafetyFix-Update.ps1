<#
.SYNOPSIS
  DYO Windows Worker - CRITICAL SAFETY FIX update, for an ALREADY-REGISTERED
  install. Double-click DYO-Worker-SafetyFix-Update.bat instead of running
  this file directly.

.DESCRIPTION
  Ships a critical safety fix to EXECUTE_FRAME, the operation that mutates
  and saves an After Effects project. This fixes a real production incident
  (2026-09-08) where EXECUTE_FRAME mutated and saved whatever project AE
  already had open - which happened to be the IMMUTABLE SOURCE .aep, left
  open by an earlier unrelated read-only job - instead of the intended
  session working copy. The source was recovered and independently
  re-verified by SHA256 before this update was prepared.

  This update makes EXECUTE_FRAME:
    - Explicitly open the session's own working-copy .aep in After Effects
      before applying any edit, using the same fixed, versioned open-project
      script INSPECT_TEMPLATE already uses (no new automation). It refuses
      to proceed - failing the job instead of guessing - unless AE's own
      self-reported open project path matches the working copy exactly.
    - Independently re-hash the immutable source .aep from disk, both
      before and after every execution, and fail the job hard the moment
      those bytes ever differ - never trusting a database-stored hash as
      proof of current file content.
    - Refuse to report success if a job claims real edits were applied but
      the working copy's own bytes never actually changed.
  ...
  Includes `npm install` (like DYO-Worker-Phase5-Update.ps1) so this update
  is correct and self-contained regardless of exactly which prior update
  this computer has already received. Still never touches .env:
  DYO_API_URL, AE_PATH, AE_MCP_PATH, and WORK_ROOT are all left exactly as
  they already are.
...
Write-Host "================================================"
Write-Host "  DYO Windows Worker - CRITICAL SAFETY FIX Update"
Write-Host "================================================"
Write-Host "This updates the DYO Worker program files on this ALREADY-REGISTERED"
Write-Host "computer with a critical safety fix to EXECUTE_FRAME. It does not ask for"
...
if (-not (Test-Path (Join-Path $sourceApp "dist\index.js"))) {
  ...
  Write-Host "Re-download the full DYO Worker safety fix update package and try again."
  exit 1
}
...
if ($npmExitCode -ne 0) {
  ...
  Write-Host "Check your internet connection and re-run DYO-Worker-SafetyFix-Update.bat."
  exit 1
}
...
if (Test-Path $sdkCheckPath) {
  ...
  Write-Host "Please run DYO-Worker-SafetyFix-Update.bat again. If this keeps happening, contact DYO."
  exit 1
}
...
Write-Host "================================================"
Write-Host "DYO Worker is running with the critical EXECUTE_FRAME safety fix, using the"
...
Write-Host "update."
Write-Host ""
Write-Host "Do NOT run another real EXECUTE_FRAME job until the server (API) side has"
Write-Host "also been redeployed with the matching schema change, exactly one DYO Worker process"
Write-Host "is confirmed running for this machine, and the restored source .aep's SHA256 has"
Write-Host "been re-verified."
Write-Host ""
