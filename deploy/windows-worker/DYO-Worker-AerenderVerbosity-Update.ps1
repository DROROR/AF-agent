<#
.SYNOPSIS
  DYO Windows Worker - aerender verbosity argument fix, for an
  ALREADY-REGISTERED install. Double-click
  DYO-Worker-AerenderVerbosity-Update.bat instead of running this file
  directly.

.DESCRIPTION
  Fixes a real 2026-09-10 incident (job ea99c6dd-359d-4c02-ab3b-01687a1d8cf3,
  AE 26.3x87): every Complete Preview / render attempt was failing
  immediately with "aerender SYNTAX ERROR: Bad value for -verbose." The
  Worker's own fixed aerender command line was passing an invalid value
  ("ERRORS_AND_WARNINGS") alongside the real -v flag - AE's own aerender
  does not recognize that value at all. This update replaces it with
  ERRORS_AND_PROGRESS, Adobe's real documented value.

  This is the ONE canonical argument builder both Complete Preview and
  the Landscape/Reels render already share, so this single fix corrects
  both. Nothing else about the aerender command line changes - the same
  project path, composition name, Render Settings/Output Module template
  names, and output path this Worker already computes.

  This is a strictly ADDITIVE/corrective change:
    ...
    - Never changes what composition, template, or output path is used -
      only the one verbosity value.
...
Write-Host "================================================"
Write-Host "  DYO Windows Worker - aerender Verbosity Fix Update"
Write-Host "================================================"
Write-Host "This updates the DYO Worker program files on this ALREADY-REGISTERED"
Write-Host "computer with a fix for the real aerender syntax error blocking every"
Write-Host "Complete Preview and render attempt. It does not ask for a registration"
...
if (-not (Test-Path (Join-Path $sourceApp "dist\index.js"))) {
  ...
  Write-Host "Re-download the full DYO Worker aerender Verbosity Fix update package and try again."
  exit 1
}
...
if ($npmExitCode -ne 0) {
  ...
  Write-Host "Check your internet connection and re-run DYO-Worker-AerenderVerbosity-Update.bat."
  exit 1
}
...
if (Test-Path $sdkCheckPath) {
  ...
  Write-Host "Please run DYO-Worker-AerenderVerbosity-Update.bat again. If this keeps happening, contact DYO."
  exit 1
}
...
Write-Host "================================================"
Write-Host "DYO Worker is running with the aerender verbosity fix, using the same"
Write-Host "DYO Worker identity this computer already had - no new registration was"
Write-Host "created. No After Effects project was opened, changed, or run against"
Write-Host "by this update."
Write-Host ""
